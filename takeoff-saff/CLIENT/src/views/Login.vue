		...
		<v-col cols="12" sm="10" md="8" lg="6">
			...
			>
				...
				<v-card-text>
					...
					<small v-if="loginTry" class="error--text d-flex justify-center">Неверный логин или пароль</small>
				</v-card-text>
...
export default {
	...
	methods: {
		...
		async login() {
			...
			let result = false
			if (this.formValid) {
				result = await this.serverLogin({email: this.email, password: this.password1})
				if (result) {
					this.$router.push('/')
				} else {
					this.loginTry = true
